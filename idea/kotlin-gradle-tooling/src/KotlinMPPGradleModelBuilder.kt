...
class KotlinMPPGradleModelBuilder : ModelBuilderService {
    ...
    private fun computeSourceSetsDeferredInfo(
        ...
        for (sourceSet in sourceSets.values) {
            (allSourceSetToCompilations[sourceSet]?.all {it.isTestModule} ?: if (!isHMPPEnabled && sourceSet.name == KotlinSourceSet.COMMON_TEST_SOURCE_SET_NAME) true else null)?.let { isTest ->
